//some config vars
var resourcePath = '~/sites/all/modules/writing_tree/';	//path to the module; Ben added a leading tilde
...
function buildInterface(){
	...
	var maxHeightEach = 0
	for (x in nodeList){maxHeightEach = maxHeightEach + 1;}
	for (x in tags){maxHeightEach = maxHeightEach + 1;}
	
	
	maxHeightEach = jQuery("#chart").height() / maxHeightEach;
	...
	for (x in nodeList){
		...
		///ohhhh jesus....its an array of objects, nobodies got time for that
...
function buildNetwork(){
	...
	force.on("tick", function(e) {
		if (e.alpha<=.05){
			...
			vis.selectAll("g.node").attr("visibility","hidden")				
		}
	...
	function filter(){
		...
		for (aNode in dataObj.nodes){	
			...
			//if (dataObj.nodes[aNode].title.toLowerCase().search('miller') != -1){
